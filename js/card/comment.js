$("#task-comment-btn").on("click",function(){
	...
	var route = systemRoute+boxId+"/comment?api_token="+api_token ;
	var data = { description:description } ;
	var task_created_by = $("#task_created_by").val() ;
	// owner=0;
	// if(task_created_by==user_id){
	// 	owner=1;
	// }

	// var notification = { task_id:boxId,owner:owner,task_created_by:task_created_by } ;
	// console.log('notification',notification);
	// socket.emit('notification',room);
...
$(document).on("click",".btn-comment-delete",function(event) { 
	...
	swal({
	  ...
	  if (result.value) {
			...
			var route = systemRoute+boxId+"/comment/"+comment_id+"?api_token="+api_token ;
			var data = "" ;
