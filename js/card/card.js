...
function createTask(data){
  ...
  //   $(".show-content",data).find('.box-id').each(function(index, el) {
...
function updateListCard(data){
	...
	if(data.task_category!=null){
		var taskCategory = data.task_category ;
		        ...
		        if($(this).val()==cardId){
		            ...
		            if ($("#room_id").length > 0){
		                ...
		                if(userEle.length >0 ){
		                    ...
		                    taskCategory.name_en+"</small>";
		                        // console.log(userEle,html);
		                        userEle.append(html);
		                    }else{
		                        userEle.find('.category-label').css({color:taskCategory.color}).text(taskCategory.name_en);
		                        userEle.append(html);
		                    }
		                }
		            }
    ...
    //---checklist
    if(data.task_checklists!=null){
    	var taskCheckList = data.task_checklists ; 
    	var checklist_success = 0 ;
    	var checklist_total = 0;
    	// console.log('taskCheckList.length',taskCheckList.length);
    	if(taskCheckList.length > 0){
    		for (var i=0;i<taskCheckList.length ;i++){
    			// console.log('taskCheckList[i].item.length',taskCheckList[i].item.length);
    			if(taskCheckList[i].item.length>0){
    				for (var j =0 ; j<taskCheckList[i].item.length; j++){ 
		    			if(taskCheckList[i].item[j].status){
				          checklist_success ++;
				        }
						checklist_total++;
		    		}
		    ...
		    $('.box-id').each(function(index, el) {
		        if($(this).val()==cardId){
		            var ele = $(this).closest('.show-content').find('.box-body');
		            var check = "<i class=\"fa fa-check-square-o\"></i> "+checklist_success+" / "+checklist_total;
		            ele.find('.card-checklist').html(check);
		        }
		    });
    	}else{
    		$('.box-id').each(function(index, el) {
    ...
    //---member
    if(data.task_members!=null){
    	var taskMembers = data.task_members ;
    	if(taskMembers.length >0){
    		var html = "";
    		for(var i=0;i<taskMembers.length;i++){
    			html += "<img src=\""+taskMembers[i].img+"\" data-id=\""+taskMembers[i].id+"\" data-toggle=\"tooltip\" title=\""+
                taskMembers[i].text+"\" class=\"img-circle assign-member-img\" height=\"25\" alt=\"User Image\">" ;
    		}
    		$('.box-id').each(function(index, el) {
	            if($(this).val()==cardId){
	                var ele = $(this).closest('.show-content').find('.box-body')
	                ele.find('.card-member').html(html);
	            }
	        });
    	}else{
    		$('.box-id').each(function(index, el) {
...
function createTaskCard(data){
	...
	if(data.due_dated_at!=null){
		var duedateShow = moment.utc(data.due_dated_at).format("D/MM/YYYY HH:mm")
		$("#task_due_date span#btn_edit_duedate").text(duedateShow);
		$("#task_due_date").show();
		duedate = new Date(data.due_dated_at);
	}
	if(data.due_dated_complete){
		$("#task_duedate_complete").prop('checked', true);
	}

	$("#task_start_date").hide();
	var startDate = new Date();
	if (data.start_task_at!=null){
		startDate = new Date(data.start_task_at);
		var startdateShow = moment.utc(data.start_task_at).format("D/MM/YYYY HH:mm")
		$("#task_start_date span#btn_edit_startdate").text(startdateShow);
		$("#task_start_date").show();
	}

	if($('.btn_duedate').length>0){
		$('.btn_duedate').data('daterangepicker').setStartDate(duedate);
		$('.btn_duedate').data('daterangepicker').setEndDate(duedate);
	}

	if($('#btn_edit_duedate').length>0){
		$('#btn_edit_duedate').data('daterangepicker').setStartDate(duedate);
		$('#btn_edit_duedate').data('daterangepicker').setEndDate(duedate);
	}
   	if($('.btn_start_task').length>0){
   		$('.btn_start_task').data('daterangepicker').setStartDate(startDate);
		$('.btn_start_task').data('daterangepicker').setEndDate(startDate);
   	}
	...
	$(".task-menu-room #room_number").text(data.room_number);
	$(".task-menu-room #room_name").text(data.room_name);
	$(".task-menu-room #room_tel").text(data.room_tel);
	$(".task-menu-room #room_email").text(data.room_email);
...
$(function() {
	...
	$("#checklist_title").on('focus', function() { $(this).select(); });
	...
	$("#btn-task-viewer").on("click", function(event) {
		var cardId = $("#current_card_id").val();
		var route = "/task/"+cardId+"/viewer?api_token="+api_token ;
	...
	$("#btn-remove-task").on("click", function(event) {
		var cardId = $("#current_card_id").val();
		var route = "/task/"+cardId+"?api_token="+api_token ;
		...
		swal({
		  ...
		  if (result.value) {
			    ajaxPromise('DELETE',route).done(function(data){
			    	...
			    	if($("#room_id").length>0){
			    		 window.location.href = $("#baseUrl").val()+"/user/"+$("#room_id").val()+"/task" ;
			    	}else{
			    		 window.location.href = $("#baseUrl").val()+"/task" ;
			    	}
...
function UpdateTask(data){
    ...
    ajaxPromise('PUT',route,data).done(function(res){
    	dfd.resolve(res);
    	socket.emit('task',res);
...
function ajaxUpdateStatus(boxId,status){
	...
	var url = $("#apiUrl").val() ;
	$.ajax({
		url: url+"/task/"+boxId+"/status?api_token="+api_token,
		type: 'PUT',
		dataType: 'json',
		data : {status:status}
	})
...
function ajaxCreateQuatation(txt,start){
	...
	var url = $("#apiUrl").val() ;
	$.ajax({
		url: $("#apiUrl").val()+'/task?api_token='+api_token,
		type: 'POST',
		dataType: 'json',
