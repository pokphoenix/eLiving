...
function createQuatationTable(data){
			...
			for (var j =0 ; j < company.length ; j++ ){
				...
				for (var k =0 ; k < companyItem.length;k++){
					...
					if (companyItem[k].company_id==company[j].company_id &&
						companyItem[k].quotation_item_id==item[i].id)
					{
						table += "<td class=\"text-center\">"+( (companyItem[k].price_per_unit==0) ? '-' : ReplaceNumberWithCommas((companyItem[k].price_per_unit).toFixed(2))  )+"</td>"+
					"<td class=\"text-center\"><span class=\"item-price-total\">"+( (companyItem[k].price==0) ? '-' :  ReplaceNumberWithCommas((companyItem[k].price).toFixed(2))  )+"</span></td>"	;
						hasCompanyItem = true;
					}
	...
	for (var k =0 ; k < company.length;k++){
		table += "<td></td><td class=\"total-price-before-vat text-center\">"+ ReplaceNumberWithCommas((company[k].price_b4_vat).toFixed(2))+"</td>";	
	}
	table += "</tr>";
	...
	for (var k =0 ; k < company.length;k++){
		table += "<td></td><td class=\"price-discount text-center text-danger\">"+((company[k].discount>0) ?  ReplaceNumberWithCommas((company[k].discount).toFixed(2)) : '-' )+"</td>";	
	}
	table += "</tr>";
	...
	for (var k =0 ; k < company.length;k++){
		table += "<td></td><td class=\"price-vat text-center\">"+ ReplaceNumberWithCommas((company[k].vat).toFixed(2))+"</td>";	
	}
	table += "</tr>";
	...
	for (var k =0 ; k < company.length;k++){
		table += "<td></td><td class=\"total-price-net text-center\"><b>"+ ReplaceNumberWithCommas((company[k].price_net).toFixed(2))+"</b></td>";	
	}
	table += "</tr>";
