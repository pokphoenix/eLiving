...
$(document).on("click",".btn-add-card",function (event) {
    ...
    var route = "/user/suggest/system?api_token="+api_token ;
    var data = { title:txt , category_id:categoryId }
    ajaxPromise('POST',route,data).done(function (data) {
      // socket.emit('task',data);
        $('.addcard-box').hide();
        $("#first_open_card").val('true');
        var boxId = data.suggest.id ;
        var res = data.suggest ;

        var card = "<div class=\"box box-solid card show-content\" "+
          " style=\"border-left: 5px solid "+res.status_color+";\">"+
                  "<div class=\"box-header\">"+
                    "<small class=\"category-label\" data-id=\""+data.suggest_category.id+"\" style=\"color:"+data.suggest_category.color+
              ";font-weight: bold;\" >&nbsp;"+
                      (($("#app_local").val()=='th') ? data.suggest_category.name_th : data.suggest_category.name_en )+"</small>"+
...
function OpenCard(title,boxId,data)
{
    ...
    window.history.pushState("object or string", title , $("#baseUrl").val()+'/suggest/system/'+boxId);
    clearTaskData();
