...
<!-- data-toggle="modal" data-target="#modal-card-content" -->
    ...
    <section class="content-header">
      <h1>
      <i class="fa fa-circle-o"></i> @lang('sidebar.work')
        <small></small>
      </h1>
      <ol class="breadcrumb">
...
<script type="text/javascript" src="{{ url('js/user/work/member.js') }}"></script>
...
var baseRoute = "/officer/work/" ;
var systemRoute = "/officer/work/" ;
...
socket.on(socketRoute, function(data){
	if (typeof data==='string'){
   	...
   	if(data.work!=null&&data.task_category!=null&&data.work_comments.length==0){
    	appendCard('#card_new',data.work.id,data.work.title,data.task_category.color,data.task_category.name_th);
    }
 	...
 	// if(data.suggest_lastest_category_id!=null){
 		if(data.task_category==null&&data.work_id==currentCardId){ //--- ถ้าข้อมูลถูกลบออก ต้องเอาออกจาก หน้า list
...
<script type="text/javascript">
	...
	$(".btn-print").on("click",function(){
		...
		var route = $("#baseUrl").val()+systemRoute+cardId+"/print";
