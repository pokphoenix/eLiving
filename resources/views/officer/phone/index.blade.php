    	...
    	<div class="box">
           <div class="box-header">
              ...
              <button class="btn btn-success btn-sm" data-toggle="modal" data-target="#exampleModal" 
               > <i class="fa fa-edit"></i> @lang('phone.edit')
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">@lang('phone.new')</h5>
...
$(function() {
  ...
  $(document).on("click",".btn-save",function(event) {  
    $(this).find('.fa-spinner').show();
      ...
      var url = "{{ url('api/'.$domainId) }}/phone/1?api_token="+api_token ;
      $.ajax({
                 type: "PUT",
                 url: url,
                 data:{text:content},
  ...
  $(document).on("submit",".delete-phone",function(event) { 
    var ele = $(this);
    swal({
      title: 'แน่ใจไหม?',
      text: "คุณต้องการลบเบอโทรนี้ทิ้งใช่หรือไม่!",
      type: 'warning',
      ...
      if (result.value) {
          var phoneId = $.trim(ele.data('id'));
          var url = "{{ url('api/'.$domainId) }}/phone/"+phoneId+"?api_token="+api_token ;
          console.log(phoneId,url);
           $.ajax({
                 type: "DELETE",
                 url: url,
                 success: function (data) {
