...
class ParkingBuy extends Model
{
    ...
    protected $fillable = ['domain_id','room_id', 'package_id','created_by','created_at','expired_at','user_buy_name','period_at','id_card_buyer','deleted_at','deleted_by','buyer_tel'];
    ...
    public static function getRemain($domainId,$packageId,$roomId,$periodAt,$exceptId =null){
    	...
    	$sql = " SELECT 
                    IFNULL( 
                    (SELECT times_limit FROM parking_package  WHERE id=$packageId )
                    ,0) - 
                    IFNULL( 
