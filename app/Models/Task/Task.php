  ...
  //               }
    ...
    public static function getTaskData($domainId,$taskId,$type=null){
    	...
    	$data['task'] = Task::where('domain_id',$domainId)
    					...
    					->first() ;
    	$data['task_id'] = $taskId;			
    	$data['task_category'] = TaskCategory::find($data['task']->category_id) ;
	...
	public static function taskStatus(){
		 return [ ['id'=>1,'text'=> (!App::isLocale('th') ? 'รายการใหม่' : 'New' ),'color'=>'#8a8a8a']
			,['id'=>2,'text'=> (!App::isLocale('th') ? 'เริ่มทำในสัปดาห์นี้' : 'To Do' ),'color'=>'#52a7e0']
			,['id'=>3,'text'=> (!App::isLocale('th') ? 'ตอบรับ' : 'Accept' ),'color'=>'#00c0ef']
			,['id'=>4,'text'=> (!App::isLocale('th') ? 'ยกเลิก' : 'Cancel' ),'color'=>'#f56954']
			,['id'=>5,'text'=> (!App::isLocale('th') ? 'กำลังดำเนินการ' : 'In Progress' ),'color'=>'#f39c12']
			,['id'=>6,'text'=> (!App::isLocale('th') ? 'รอดำเนินการ' : 'Pending' ),'color'=>'#605ca8']
			,['id'=>7,'text'=> (!App::isLocale('th') ? 'เสร็จแล้ว' : 'Done' ),'color'=>'#00a65a']
		 ] ;
	}
