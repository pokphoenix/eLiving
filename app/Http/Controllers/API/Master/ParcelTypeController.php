...
use App\Models\Master\ParcelType;
use App\User;
use Auth;
use DB;
...
class ParcelTypeController extends ApiController
{
    /*
    |--------------------------------------------------------------------------
    | Login Controller
    ...
    */
    ...
    private $route = 'parcel_type';
    private $table = 'master_parcel_type';
    ...
    {
        $data[$this->route]  = ParcelType::all();
    ...
    public function destroy(Request $request, $domainId, $id)
    {
        ...
        $post = $request->except('api_token', '_method');
        $query = ParcelType::find($id)->delete();
        return $this->respondWithItem(['id'=>$id]);
    }
