...
use App\Models\Master\SupplyType;
use App\User;
use Auth;
use DB;
...
class SupplyTypeController extends ApiController
{
    /*
    |--------------------------------------------------------------------------
    | Login Controller
    ...
    */
    ...
    private $route = 'supply_type';
    private $table = 'master_supplies_type';
    ...
    {
        $data[$this->route]  = SupplyType::all();
    ...
    public function destroy(Request $request, $domainId, $id)
    {
        ...
        $post = $request->except('api_token', '_method');
        $query = SupplyType::find($id)->delete();
        return $this->respondWithItem(['id'=>$id]);
    }
