    ...
    */
    ...
    public function index($domainId){
        $sql = "SELECT pu.*,u.first_name,u.last_name
                ...
                WHERE  DATE_FORMAT(now() ,'%Y-%m') = DATE_FORMAT(used_date ,'%Y-%m')
                AND pu.domain_id = $domainId
                AND pu.used_date is not null";
        $data['parking_cancel']  = DB::select(DB::raw($sql));
       
        $sql = "SELECT pu.*,u.first_name,u.last_name
                ,pv.PROVINCE_NAME as province_name
                ,CONCAT( IFNULL(r.name_prefix,''),IFNULL(r.name,''),IFNULL(r.name_surfix,'') ) as room_name
                FROM parking_history as ph 
                JOIN parking_use as pu 
                ON pu.id = ph.parking_use_id
                JOIN rooms as r 
                ON r.id = pu.room_id
                JOIN users as u 
